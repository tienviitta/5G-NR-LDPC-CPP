...
public:
  ...
  // the i-th layer stores all the connections(edges) to the i-th check node in
  // a tanner graph
...
public:
  ...
  // decoding
  std::vector<bool> decode(const std::vector<double> &llr,
                           const unsigned nMaxIter);
  // rate matching
  std::vector<bool> rateMatch(const std::vector<bool> &bitsIn,
                              std::size_t nOfBitOut);
  // rate recovery
...
public:
  ...
  static std::vector<std::vector<bool>> makeParityCheckMatrix(uint8_t BGn,
                                                              uint16_t Zc);

private:
  // core decoding module
  std::vector<std::vector<double>>
  checkNodeOperation(const std::vector<std::vector<double>> &msgIn);
  template <typename T>
  inline std::vector<std::size_t> sort_indexes(const std::vector<T> &v);
  std::vector<std::vector<double>>
  transposeMat(const std::vector<std::vector<double>> &mat);

  // wrapper functions to enhance readability
  template <typename T>
  inline std::vector<T> circShift(const std::vector<T> &vecIn,
                                  const unsigned nShifts);
  inline std::vector<bool> eleWiseXor(const std::vector<bool> &vec1,
                                      const std::vector<bool> &vec2);
  inline std::vector<double> eleWisePlus(const std::vector<double> &vec1,
                                         const std::vector<double> &vec2);
  inline std::vector<double> eleWiseMinus(const std::vector<double> &vec1,
                                          const std::vector<double> &vec2);
};
...
*/
extern int32_t mcsTbl[32][2];

/**
Table 5.1.3.2-1: TBS for N_info <= 3824
*/
extern int32_t tbsTbl[93];
