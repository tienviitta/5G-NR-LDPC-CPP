...
nrLDPC::nrLDPC(size_t infoLen, double codeRate) {
  ...
  if (mBGn == 1) {
    mLayers = {
        {0, 19},    {19, 38},   {38, 57},   {57, 76},   {76, 79},   {79, 87},
        {87, 96},   {96, 103},  {103, 113}, {113, 122}, {122, 129}, {129, 137},
        {137, 144}, {144, 150}, {150, 157}, {157, 164}, {164, 170}, {170, 176},
        {176, 182}, {182, 188}, {188, 194}, {194, 200}, {200, 205}, {205, 210},
        {210, 216}, {216, 221}, {221, 226}, {226, 230}, {230, 235}, {235, 240},
        {240, 245}, {245, 250}, {250, 255}, {255, 260}, {260, 265}, {265, 270},
        {270, 275}, {275, 279}, {279, 284}, {284, 289}, {289, 293}, {293, 298},
        {298, 302}, {302, 307}, {307, 312}, {312, 316}};
  } else {
    mLayers = {
        {0, 8},     {8, 18},    {18, 26},   {26, 36},   {36, 40},   {40, 46},
        {46, 52},   {52, 58},   {58, 62},   {62, 67},   {67, 72},   {72, 77},
        {77, 81},   {81, 86},   {86, 91},   {91, 95},   {95, 100},  {100, 105},
        {105, 109}, {109, 113}, {113, 117}, {117, 121}, {121, 124}, {124, 128},
        {128, 132}, {132, 135}, {135, 140}, {140, 143}, {143, 147}, {147, 150},
        {150, 155}, {155, 158}, {158, 162}, {162, 166}, {166, 170}, {170, 174},
        {174, 178}, {178, 181}, {181, 185}, {185, 189}, {189, 193}, {193, 197}};
  }
}
...
vector<bool> nrLDPC::encode(const vector<bool> &msg) {
  ...
  for (unsigned i = 0; i < 4; i++) {
    for (unsigned edgeIdx = mLayers[i].edgeStart; edgeIdx < mLayers[i].edgeEnd;
         edgeIdx++) {
  ...
  rotate(cWord[Kb].begin(), cWord[Kb].begin() + (mZc - shiftP0),
         cWord[Kb].end());
  ...
  for (unsigned i = 0; i < 3; i++) {
    for (unsigned edgeIdx = mLayers[i].edgeStart; edgeIdx < mLayers[i].edgeEnd;
      ...
      if (vNodeIdx <= Kb + i) {
        cWord[Kb + i + 1] =
            eleWiseXor(cWord[Kb + i + 1], circShift(cWord[vNodeIdx], nShifts));
      }
    }
  }
  ...
  for (unsigned i = 4; i < totLayers; i++) {
    ...
    for (unsigned edgeIdx = mLayers[i].edgeStart;
         edgeIdx < mLayers[i].edgeEnd - 1; edgeIdx++) {
      vNodeIdx = mEdges[edgeIdx].vNodeIdx;
      nShifts = mEdges[edgeIdx].nShifts;
      cWord[Kb + i] =
          eleWiseXor(cWord[Kb + i], circShift(cWord[vNodeIdx], nShifts));
    }
  }
...
vector<bool> nrLDPC::decode(const vector<double> &softBitsIn,
                            const unsigned nMaxIter) {
  //------------------------------------------------------------------------------------------------------
  // [ref] Hocevar, D.E. "A reduced complexity decoder architecture via layered
  // decoding of LDPC codes." In IEEE Workshop on Signal Processing Systems,
  // 2004. SIPS 2004.
  //------------------------------------------------------------------------------------------------------
  ...
  for (unsigned i = 0; i < mN / mZc; i++) {
    LLR[i] = vector<double>(softBitsIn.begin() + i * mZc,
  ...
  unsigned nMaxLayer;
  if (mBGn == 1)
    // assume tx bits length =  ceil(kBar/R), alternatively can use all
    // layers(slower)
  ...
  // initialize msg from check nodes to vector nodes, each edge correspond a
  // message
...
nrLDPC::checkNodeOperation(const vector<vector<double>> &msgIn) {
  // ------------------------------------------------------------------------------------------------
  // [ref] Chen, Jinghu, R.M. Tanner, C. Jones, and Yan Li. "Improved min-sum
  // decoding algorithms for irregular LDPC codes." In Proceedings.
...
vector<double> nrLDPC::rateRecover(const vector<double> &softBitsIn) {
  ...
  softBitsOut.insert(softBitsOut.end(), rxBufferRing.begin(),
                     rxBufferRing.begin() + mKBar - 2 * mZc);
  // fillers
  ...
  softBitsOut.insert(softBitsOut.end(), rxBufferRing.begin() + mKBar - 2 * mZc,
                     rxBufferRing.end());
...
vector<vector<bool>> nrLDPC::makeParityCheckMatrix(uint8_t BGn,
                                                   const uint16_t Zc) {
  ...
  for (unsigned i = 0; i < numOfShifts; i++) {
    ...
    rotate(shiftVecOne.begin(), shiftVecOne.begin() + shiftCoeff,
           shiftVecOne.end());
...
template <typename T>
inline vector<T> nrLDPC::circShift(const std::vector<T> &vecIn,
...
inline vector<bool> nrLDPC::eleWiseXor(const vector<bool> &vec1,
                                       const vector<bool> &vec2) {
  vector<bool> vecOut = vec1;
  transform(vec1.begin(), vec1.end(), vec2.begin(), vecOut.begin(),
            bit_xor<>{});
  return vecOut;
}

inline vector<double> nrLDPC::eleWisePlus(const vector<double> &vec1,
...
inline vector<double> nrLDPC::eleWiseMinus(const vector<double> &vec1,
                                           const vector<double> &vec2) {
  vector<double> vecOut = vec1;
...
int32_t tbsCalc(int32_t nRePrb, int32_t nPrb, double cRate, int32_t mod,
                int32_t nLayers) {
  // 1) The UE shall first determine the number of REs (NRE) within the slot.
  const int32_t nReSlot = fmin(156, nRePrb) * nPrb;
  // 2) Unquantized intermediate variable (N_info)
  const double nInfo = nReSlot * cRate * mod * nLayers;
  if (nInfo <= 3824) {
    // 3) When N_info <= 3824, TBS is determined as follows
    double n = fmax(3.0, floor(log2(nInfo)) - 6.0);
    double nInfop = fmax(24.0, pow(2, n) * floor(nInfo / pow(2, n)));
    int32_t i = 0;
    // Use Table 5.1.3.2-1 find the closest TBS that is not less than N_info'
    while (nInfop > tbsTbl[i]) {
      i++;
    }
    // printf("nInfo: %.2f, n: %.2f, nInfop: %.4f, i: %d\n", nInfo, n, nInfop,
    // i);
    return tbsTbl[i];
  } else {
    // 4) When N_info > 3824, TBS is determined as follows.
    double n = floor(log2(nInfo - 24.0)) - 5.0;
    // Npinfo = max(3840, (2**n) * np.round((Ninfo - 24) / (2**n)))
    double nInfop = fmax(3840.0, pow(2, n) * round((nInfo - 24.0) / pow(2, n)));
    // printf("nInfo: %.2f, n: %.2f, nInfop: %.4f, ", nInfo, n, nInfop);
    if (cRate <= (1.0 / 4.0)) {
      int32_t C = (int32_t)(ceil(nInfop + 24.0) / 3826.0);
      // printf("C: %d\n", C);
      return (int32_t)(8 * C * ceil((nInfop + 24.0) / (8 * C)));
    } else {
      if (nInfop > 8424.0) {
        int32_t C = (int32_t)(ceil((nInfop + 24.0) / 8424.0));
        // printf("C: %d\n", C);
        return (int32_t)(8 * C * ceil((nInfop + 24.0) / (8 * C)) - 24);
      } else {
        return (int32_t)(8 * ceil((nInfop + 24.0) / (8)) - 24);
      }
    }
  }
}
