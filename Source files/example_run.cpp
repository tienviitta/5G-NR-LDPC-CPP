...
void example_run_LDPC() {
  ...
  int iMcs = 0;
  int32_t mod = mcsTbl[iMcs][0];
  int32_t cRatep = mcsTbl[iMcs][1];
  double rate = cRatep / 1024.0; // code rate
  const int32_t tbsA = tbsCalc(156, 1, rate, mod, 1);
  unsigned K = tbsA + 16;      // information length
  int M = mod * nPrb * nRePrb; // rate matching length
  cout << "I_MCS: " << iMcs << ", TBS: " << tbsA << ", rate: " << rate
       << ", K: " << K << ", M: " << M << "\n";
  ...
  transform(EsN0_dB.begin(), EsN0_dB.end(), N0.begin(),
            [](const float &x) { return pow(10.0, -x / 10.0); });
  ...
  for (unsigned i = 0; i < N0.size(); i++) {
    // print progress
    char str[100];
    sprintf(str, "\nNow running EsN0: %.2f dB [%d of %lu]", EsN0_dB[i], i + 1,
            N0.size());
    cout << str << endl;
